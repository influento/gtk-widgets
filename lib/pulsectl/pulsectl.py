...
class Pulse(object):
	...
	sink_input_kill = _pulse_method_call(c.pa.context_kill_sink_input, None)
	source_output_kill = _pulse_method_call(c.pa.context_kill_source_output, None)
	...
	def get_peak_sample(self, source, timeout, stream_idx=None):
		...
		def read_cb(s, bs, userdata):
			buff, bs = c.c_void_p(), c.c_size_t(bs)
